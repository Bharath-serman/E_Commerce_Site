...
export default function AdminDashboard() {
  ...
  const handleSearch = () => {
    ...
    const filtered = orders.filter(order => {
      ...
      return (
        transId.toLowerCase().includes(term) ||
                ...
                <td className="px-6 py-4 whitespace-nowrap">
                  <div className="flex flex-col">
                    <span className="text-[10px] text-zinc-900 font-mono font-bold">
                      {order.transactionId || `#${order.stripeSessionId.slice(-8).toUpperCase()}`}
                    </span>
                    {!order.transactionId && (
                      <span className="text-[9px] text-zinc-400 uppercase tracking-tighter">Legacy Reference</span>
                    )}
                  </div>
                </td>
                <td className="px-6 py-4 whitespace-nowrap">
